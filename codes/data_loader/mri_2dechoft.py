...
class MRI2DEchoFt(TFDatasetBase):
    def __init__(self, all_config={}, mode='train'):

        self.mode = mode
        self.dataset_name   = 'mri_2dechoft'
        ...
        ft_dict = {'fileType':'ft',     'dataType':self.outputType, 'dataDim':'NXYE',   'norm': None, 'rm_skull': self.rm_skull, 'mask_type':self.mask_type}
        mask_dict = {'fileType':'mask', 'dataType':self.outputType, 'dataDim':'NXYC',   'norm':None, 'rm_skull': self.rm_skull, 'mask_type':self.mask_type}
    ...
    def __getitem__(self, item):
        ...
        return ipt.astype(np.float32), gt.astype(np.float32), ft.astype(np.float32), skull_mask.astype(np.float32)
...
def mri_2dechoft(config, mode: str):
    ...
    if mode == 'train':
        train_dataset = MRI2DEchoFt(all_config=config, mode='train')
        valid_dataset = MRI2DEchoFt(all_config=config, mode='valid')
        return train_dataset, valid_dataset
    if mode == 'test':
        test_dataset = MRI2DEchoFt(all_config=config, mode='test')
        return test_dataset
